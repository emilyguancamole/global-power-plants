...
const CountryTable = () => {
    ...
    useEffect(() => {
        fetchTop25().then((data: CountryCapacityType[]) => {
    ...
    const rows = countries.map((country, index) => ({ // assign an index to each `country` item
        id: index,
        ...country,
    }));
    ...
    const columns: GridColDef[] = [
        { field: "rank", headerName:"Rank", width: 80, type:"number",
            valueGetter: (val, row) => {return countries.findIndex(country => country.country_code === row.country_code) + 1; }
        },
        // { field: "country_code", headerName: "Code", width: 100 },
        { field: "country_name", headerName: "Country", width: 200 },
        { field: "tot_capacity", headerName: "Total Capacity (MW)", type: "number",     
            width: 200,
            valueFormatter: (value: number) => {return Math.round(value).toString();}
        },
    ];
