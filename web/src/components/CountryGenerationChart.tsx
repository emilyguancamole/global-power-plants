...
import type { GenerationOverTimeType, GenerationChartDataPoint } from "@/data/types";
...
const CountryGenerationChart = ({ selectedCountries }: CountryGenerationChartProps) => {
  
  const [chartData, setChartData] = useState<GenerationChartDataPoint[]>([]);

  const countriesToShow = selectedCountries && selectedCountries.length > 0 ? selectedCountries : ["USA", "CAN"];

  useEffect(() => {
    const dataObj: Record<string, GenerationOverTimeType[]> = {}; //* countrycode->[{2003,year_gen}, {2004,year_gen}, ...]
    const fetchAll = async () => {
      for (const code of countriesToShow) { //* "of" to loop thru values
        try {
